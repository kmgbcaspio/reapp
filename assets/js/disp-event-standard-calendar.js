$('#cb-calendar').fullCalendar({
	...
	events: '../assets/ws/event-get.php',
});
...
function updateEvent( event, revertFunc ){
	...
	var url = '../assets/ws/event-update.php?User_ID=' + CONST_User_ID + '&Event_ID=' + event.id + '&Date_Start=' + dateStart + '&Date_End=' + dateEnd + '&All_Day=' + event.allDay;
	$.get( url, function() {
	})
		.done( function() {
			$('.loader').hide();
